...
And then the polling averages, weighted by each state's number of electoral votes, ignoring the states for which a candidate has not been represented by a head-to-head poll with Donald Trump (see NB below regarding the shortcomings of averaging the swing states together):
